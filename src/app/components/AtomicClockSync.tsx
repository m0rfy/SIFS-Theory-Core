...
interface AtomicClockSyncProps {
  onFrequencyChange?: (value: number) => void;
  onTimeDilationDeltaChange?: (value: number) => void;
  onAmplitudeChange?: (value: number) => void;
}

export const AtomicClockSync: React.FC<AtomicClockSyncProps> = ({
  onFrequencyChange,
  onTimeDilationDeltaChange,
  onAmplitudeChange,
}) => {
  ...
  const currentEpsilonRef = useRef<number>(0);
  ...
  const getScaleMultiplier = (unit: TimeUnit): bigint => {
    ...
    switch (unit) {
      ...
      case "seconds":
        return sec; // Real time (1s per 1s)
      case "minutes":
        return 60n * sec;
      case "minutes_2":
        return 120n * sec;
      ...
      case "weeks":
        return 604800n * sec;
      case "months":
        return 2592000n * sec; // 30 days
      case "years":
        return 31536000n * sec; // 365 days
      default:
        return sec;
    }
  };
  ...
  const animate = useCallback(
    (time: number) => {
      ...
      const effectiveSpeed =
        scaleMult * BigInt(Math.floor(speedMultiplier * 100)); // Maintain precision
      const deltaSimAtto =
        (BigInt(Math.round(deltaTimeMs * 10)) * effectiveSpeed) / 1000000n; // Adjusted for multiplier math
      ...
      setState((prev) => {
        ...
        const metricFactor =
          1.0 +
          amplitude[0] * 0.000001 * Math.cos(2 * Math.PI * frequency[0] * t);

        const epsilon = metricFactor - 1.0;
        // Сохраняем текущий epsilon для передачи в spatial connector
        currentEpsilonRef.current = epsilon;
    ...
    [isPlaying, timeScale, amplitude, frequency, onTimeDilationDeltaChange]
  );
  ...
  useEffect(() => {
    onFrequencyChange?.(frequency[0]);
  }, [frequency, onFrequencyChange]);

  useEffect(() => {
    onAmplitudeChange?.(amplitude[0]);
  }, [amplitude, onAmplitudeChange]);

  // Notify parent component of time dilation delta changes (outside of setState)
  // Используем epsilon из animate callback через ref
  useEffect(() => {
    // Обновляем значение из ref, который обновляется в animate callback
    const epsilon = currentEpsilonRef.current;
    onTimeDilationDeltaChange?.(epsilon);
  }, [state.globalTime, onTimeDilationDeltaChange]);
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <CardDescription className="text-slate-400">
              Синхронизация с локальным временем • Точность до аттосекунд (10⁻¹⁸
              с)
            </CardDescription>
          </div>
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
            <Select
              value={timeScale}
              onValueChange={(v: TimeUnit) => setTimeScale(v)}
            >
            ...
            <Button
              ...
              className="w-10 h-10 border-slate-700 hover:bg-slate-800 shrink-0"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900/50 rounded-xl p-6 border border-slate-800 relative overflow-hidden group">
              ...
              <div className="flex flex-col gap-1 mb-2">
                ...
                <div className="font-mono text-3xl md:text-4xl font-bold text-slate-200 tracking-tight">
                  {tGlobal.time}.
                  <span className="text-slate-400">{tGlobal.ms}</span>
                </div>
              </div>
              {isHighPrecisionVisible && (
                <div className="grid grid-cols-5 gap-1 text-[10px] md:text-xs font-mono text-slate-500 border-t border-slate-800/50 pt-2 transition-opacity duration-500">
                  <div className="text-center p-1 rounded bg-slate-800/20 text-slate-400">
                    {tGlobal.micro}
                  </div>
                  <div className="text-center p-1 rounded bg-slate-800/20 text-slate-500">
                    {tGlobal.nano}
                  </div>
                  <div className="text-center p-1 rounded bg-slate-800/20 text-slate-600">
                    {tGlobal.pico}
                  </div>
                  <div className="text-center p-1 rounded bg-slate-800/20 text-slate-700">
                    {tGlobal.femto}
                  </div>
                  <div className="text-center p-1 rounded bg-slate-800/20 text-slate-800">
                    {tGlobal.atto}
                  </div>
                </div>
              )}
          ...
          <div className="space-y-2">
            ...
            <div className="bg-slate-900/80 rounded-xl p-6 border border-blue-900/30 relative overflow-hidden shadow-[0_0_30px_-10px_rgba(59,130,246,0.1)]">
              ...
              <div className="flex flex-col gap-1 mb-2">
                <div className="text-sm text-blue-500/50 font-mono">
                  {tAtomic.date}
                </div>
                <div className="font-mono text-3xl md:text-4xl font-bold text-blue-400 tracking-tight drop-shadow-[0_0_8px_rgba(96,165,250,0.5)]">
                  {tAtomic.time}.
                  <span className="text-blue-600">{tAtomic.ms}</span>
                </div>
              </div>
              {isHighPrecisionVisible && (
                <div className="grid grid-cols-5 gap-1 text-[10px] md:text-xs font-mono text-blue-500/70 border-t border-blue-500/10 pt-2 transition-opacity duration-500">
                  <div className="text-center p-1 rounded bg-blue-500/5 text-blue-300">
                    {tAtomic.micro}
                  </div>
                  <div className="text-center p-1 rounded bg-blue-500/5 text-blue-400">
                    {tAtomic.nano}
                  </div>
                  <div className="text-center p-1 rounded bg-blue-500/5 text-blue-500">
                    {tAtomic.pico}
                  </div>
                  <div className="text-center p-1 rounded bg-blue-500/5 text-blue-600">
                    {tAtomic.femto}
                  </div>
                  <div className="text-center p-1 rounded bg-blue-500/5 text-blue-700">
                    {tAtomic.atto}
                  </div>
                </div>
              )}
          ...
          <div className="md:col-span-1 space-y-6">
            ...
            <div className="p-4 bg-slate-900 rounded-lg border border-slate-800">
              ...
              <div className="text-2xl font-bold text-slate-200">
                {(
                  Number(state.atomicTime - state.globalTime) / 1000000
                ).toFixed(6)}{" "}
                <span className="text-sm text-slate-500">ns</span>
              </div>
            </div>
          </div>
